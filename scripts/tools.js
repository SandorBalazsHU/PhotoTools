...
class Weather {
    ...
    static getLocalWeather(position) {
        return fetch(`https://api.openweathermap.org/data/2.5/weather?lat=${position.coords.latitude}&lon=${position.coords.longitude}&units=${Weather.UNIT}&APPID=${Weather.APIKEY}`).then(data => data.json());
    }
    static getWeatherForecast(position) {
        return fetch(`https://api.openweathermap.org/data/2.5/forecast?lat=${position.coords.latitude}&lon=${position.coords.longitude}&units=${Weather.UNIT}&APPID=${Weather.APIKEY}`).then(data => data.json());
    }
}
